...
class GeneratorTest extends TestCase {
	...
	public function testGenerator(): void {
		$generator = new SnowflakeGenerator(new TimeFactory(), $this->sequence);
		...
		// Check serverId
		$this->assertEquals(Util::getServerId(), $data->getServerId());
	}
	...
	public function testGeneratorWithFixedTime(string $date, int $expectedSeconds, int $expectedMilliseconds): void {
		...
		$generator = new SnowflakeGenerator($timeFactory, $this->sequence);
		$data = $this->decoder->decode($generator->nextId());
